...
(defun term-color-for-G()
  (setq term-default-bg-color "grey4")
  (setq term-default-fg-color "grey87")
  )
...
(defun color-theme-apple ()
  ...
  (color-theme-install
   '(color-theme-apple
     ...
     (mode-line ((t (:background "gray70" :foreground "black" :box (:line-width 1 :color "gray75" :style released-button :weight bold)))))
	 (mode-line-buffer-id ((t (:foreground "blue3" :bold t))))
     (mode-line-inactive ((t (:background "gray90" :foreground "gray20" :weight light :box (:line-width 1 :color "gray70" :weight normal)))))
     ...
     (semantic-highlight-edits-face ((t (:background "grey4"))))
     ;; same as comment
